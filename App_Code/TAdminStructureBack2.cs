...
/// </summary>
public class TAdminStructureBack2
{
    ...
    public TAdminStructureBack2 parent;		// Verweis auf übergeordneten Einheit
    ...
    public TAdminStructureBack2(int aOrgID, TAdminStructureBack2 aParent, int aLevel, string aProject)
    {
        if (aOrgID == -1)
        {
            // Root-Element nehmen;
            SqlDB dataReader1 = new SqlDB("SELECT OrgID from structureBack2 WHERE topOrgID='0'", aProject);
            if (dataReader1.read())
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort, filter from structureBack2 where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        hasChilds = false;
        dataReader = new SqlDB("SELECT orgID from structureBack2 where topOrgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
            hasChilds = true;
        }
    ...
    public void Reload()
    {
        ...
        // Namen der Unit aus der Datenbank lesen
        SqlDB dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort, filter from structureBack2 where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            orgDisplayName = dataReader.getString(2);
            orgDisplayNameShort = dataReader.getString(3);
        ...
        if (expandedNav)
        {
            foreach (TAdminStructureBack2 tempStructure in childs)
            {
    ...
    public void getChilds(string aProjectID)
    {
        awayChilds();
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structureBack2 where topOrgID='" + this.OrgID + "'", projectID);
        while (dataReader.read())
        {
            TAdminStructureBack2 newChild = new TAdminStructureBack2(dataReader.getInt32(0), this, this.level + 1, aProjectID);
    ...
    public static void getOrgIDList(int aOrgID, ref ArrayList aOrdIDList, string aProjectID)
    {
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structureBack2 where topOrgID='" + aOrgID + "'", aProjectID);
        while (dataReader.read())
        {
    ...
    public static int getTopOrgID(int aOrgID, string aProjectID)
    {
        int result = 0;
        SqlDB dataReader;
        dataReader = new SqlDB("SELECT topOrgID FROM structureBack2 where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {

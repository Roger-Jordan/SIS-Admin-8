...
public class TAdminStructure2
{
    ...
    public TAdminStructure2 parent;		// Verweis auf übergeordneten Einheit
    ...
    public TAdminStructure2(int aOrgID, TAdminStructure2 aParent, int aLevel, string aProject)
    {
        if (aOrgID == -1)
        {
            // Root-Element nehmen;
            SqlDB dataReader1 = new SqlDB("SELECT OrgID from structure2 WHERE topOrgID='0'", aProject);
            if (dataReader1.read())
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort from structure2 where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        dataReader.close();
        // OrgManager
        dataReader = new SqlDB("select number, numberTotal, numberSum, numberSumInput, status, backComparison, locked from orgmanager_structure2 where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        dataReader.close();
        values.Clear();
        dataReader = new SqlDB("SELECT fieldID, value FROM orgmanager_structure2fieldsvalues WHERE orgID='" + aOrgID + "'", aProject);
        ...
        dataReader = new SqlDB("SELECT COUNT(employeeID) FROM orgmanager_employee WHERE orgIDAdd2='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        // FollowUp
        dataReader = new SqlDB("select workshop, mail, livepdf, maxpdfLevel, communicationfinished, communicationfinsheddate, participants from followup_structure2 where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        // Download
        // Response
        dataReader = new SqlDB("select number, target from response_structure2 where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        hasChilds = false;
        dataReader = new SqlDB("SELECT orgID from structure2 where topOrgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
            hasChilds = true;
        }
    ...
    public TAdminStructure2(int aOrgID, string aProject)
    {
        // einzelnes Strukturelement einlesen
        OrgID = aOrgID;
    ...
    public void Reload()
    {
        ...
        // Namen der Unit aus der Datenbank lesen
        SqlDB dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort from structure2 where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            orgDisplayName = dataReader.getString(2);
            orgDisplayNameShort = dataReader.getString(3);
        ...
        dataReader.close();
        // OrgManager
        dataReader = new SqlDB("select number, numberTotal, numberSum, numberSumInput, status, backComparison, locked from orgmanager_structure2 where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberEmployees = dataReader.getInt32(0);
            numberEmployeesTotal = dataReader.getInt32(1);
        ...
        dataReader.close();
        values.Clear();
        dataReader = new SqlDB("SELECT fieldID, value FROM orgmanager_structure2fieldsvalues WHERE orgID='" + OrgID + "'", projectID);
        while (dataReader.read())
        {
            TKeyValue tempKeyValue = new TKeyValue();
            tempKeyValue.key = dataReader.getString(0);
        ...
        // Anzahl eingetragen Mitarbeiter der Einheit ermitteln
        dataReader = new SqlDB("SELECT COUNT(employeeID) FROM orgmanager_employee WHERE orgIDAdd2='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberEmployeesInput = dataReader.getInt32(0);
        }
        dataReader.close();
        // FollowUp
        dataReader = new SqlDB("select workshop, mail, livepdf, maxpdfLevel, communicationfinished, communicationfinsheddate, participants from followup_structure2 where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            workshop = dataReader.getBool(0);
            mailAdress = dataReader.getString(1);
        ...
        // Download
        // Response
        dataReader = new SqlDB("select number from response_structure2 where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberIst = dataReader.getInt32(0);
        }
        ...
        if (expandedNav)
        {
            foreach (TAdminStructure2 tempStructure2 in childs)
            {
                tempStructure2.Reload();
            }
        }
    }
    /// <summary>
    ...
    public void getChilds(string aProjectID)
    {
        awayChilds();
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure2 where topOrgID='" + this.OrgID + "'", projectID);
        while (dataReader.read())
        {
            TAdminStructure2 newChild = new TAdminStructure2(dataReader.getInt32(0), this, this.level + 1, aProjectID);
    ...
    public static void getOrgIDList(int aOrgID, ref ArrayList aOrdIDList, string aProjectID)
    {
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure2 where topOrgID='" + aOrgID + "'", aProjectID);
        while (dataReader.read())
        {
    ...
    {
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure2 where topOrgID='" + aOrgID.ToString() + "'", aProjectID);
        while (dataReader.read())
        {
            int singleSollOrg = 0;
            int singleSollRes = 0;
        ...
        dataReader = new SqlDB("select number, numberSumInput from orgmanager_structure2 where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
            aSollOrg = aSollOrg + dataReader.getInt32(0);
        }
        dataReader.close();
        dataReader = new SqlDB("select count(employeeID) from orgmanager_employee where orgIDAdd2='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
            aIstOrgInputEmployee = aIstOrgInputEmployee + dataReader.getInt32(0);
        }
        dataReader.close();
        dataReader = new SqlDB("select number, target from response_structure2 where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
        ...
        dataReader.execSQLwithParameter("UPDATE orgmanager_structure2 SET numbersum=@numbersum, numbersumInput=@numbersumInput WHERE orgID=@orgID", parameterList);
        ...
        parameterList.addParameter("orgID", "int", aOrgID.ToString());
        dataReader = new SqlDB(aProjectID);
        dataReader.execSQLwithParameter("UPDATE response_structure2 SET numberSum=@numberSum, targetSum=@targetSum WHERE orgID=@orgID", parameterList);
    }
    /// <summary>
    ...
    public static void recalc(string aProjectID)
    {
        ...
        // Root-Object finden
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure2 where topOrgID='0'", aProjectID);
        if (dataReader.read())
        {
    ...
    public static bool getTopOrgIDList(int aOrgID, ref ArrayList aOrgIDList, string aProjectID)
    {
        ...
        while (aOrgID != 0)
        {
            dataReader = new SqlDB("select topOrgID from structure2 where orgID='" + aOrgID + "'", aProjectID);
            if (dataReader.read())
            {
    ...
    public static int getTopOrgID(int aOrgID, string aProjectID)
    {
        int result = 0;
        SqlDB dataReader;
        dataReader = new SqlDB("SELECT topOrgID FROM structure2 where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
    ...
    public static void getOrgName(int aOrgID, ref string displayName, ref string displayNameShort, string aProjectID)
    {
        ...
        displayNameShort = "";
        SqlDB dataReader;
        dataReader = new SqlDB("select displayname, displaynameshort from structure2 where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {

...
/// </summary>
public class TAdminStructureBack
{
    ...
    public TAdminStructureBack parent;		// Verweis auf übergeordneten Einheit
    ...
    public TAdminStructureBack(int aOrgID, TAdminStructureBack aParent, int aLevel, string aProject)
    {
        if (aOrgID == -1)
        {
            // Root-Element nehmen;
            SqlDB dataReader1 = new SqlDB("SELECT OrgID from structureBack WHERE topOrgID='0'", aProject);
            if (dataReader1.read())
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort, filter from structureBack where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        hasChilds = false;
        dataReader = new SqlDB("SELECT orgID from structureBack where topOrgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
            hasChilds = true;
        }
    ...
    public void Reload()
    {
        ...
        // Namen der Unit aus der Datenbank lesen
        SqlDB dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort, filter from structureBack where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            orgDisplayName = dataReader.getString(2);
            orgDisplayNameShort = dataReader.getString(3);
        ...
        if (expandedNav)
        {
            foreach (TAdminStructureBack tempStructure in childs)
            {
    ...
    public void getChilds(string aProjectID)
    {
        awayChilds();
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structureBack where topOrgID='" + this.OrgID + "'", projectID);
        while (dataReader.read())
        {
            TAdminStructureBack newChild = new TAdminStructureBack(dataReader.getInt32(0), this, this.level + 1, aProjectID);
    ...
    public static void getOrgIDList(int aOrgID, ref ArrayList aOrdIDList, string aProjectID)
    {
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structureBack where topOrgID='" + aOrgID + "'", aProjectID);
        while (dataReader.read())
        {
    ...
    public static int getTopOrgID(int aOrgID, string aProjectID)
    {
        int result = 0;
        SqlDB dataReader;
        dataReader = new SqlDB("SELECT topOrgID FROM structureBack where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {

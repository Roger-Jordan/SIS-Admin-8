...
public class TAdminStructure
{
    ...
    public TAdminStructure parent;		// Verweis auf übergeordneten Einheit
    ...
    public TAdminStructure(int aOrgID, TAdminStructure aParent, int aLevel, string aProject)
    {
        if (aOrgID == -1)
        {
            // Root-Element nehmen;
            SqlDB dataReader1 = new SqlDB("SELECT OrgID from structure WHERE topOrgID='0'", aProject);
            if (dataReader1.read())
        ...
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort from structure where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        dataReader.close();
        // OrgManager
        dataReader = new SqlDB("select number, numberTotal, numberSum, numberSumInput, status, backComparison, locked from orgmanager_structure where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        dataReader.close();
        values.Clear();
        dataReader = new SqlDB("SELECT fieldID, value FROM orgmanager_structurefieldsvalues WHERE orgID='" + aOrgID + "'", aProject);
        ...
        dataReader = new SqlDB("SELECT COUNT(employeeID) FROM orgmanager_employee WHERE orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        // FollowUp
        dataReader = new SqlDB("select workshop, mail, livepdf, maxpdfLevel, communicationfinished, communicationfinsheddate, participants from followup_structure where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        // Download
        // Response
        dataReader = new SqlDB("select number, target from response_structure where orgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
        ...
        hasChilds = false;
        dataReader = new SqlDB("SELECT orgID from structure where topOrgID='" + aOrgID + "'", aProject);
        if (dataReader.read())
        {
            hasChilds = true;
        }
    ...
    public TAdminStructure(int aOrgID, string aProject)
    {
        // einzelnes Strukturelement einlesen
        OrgID = aOrgID;
    ...
    public void Reload()
    {
        ...
        // Namen der Unit aus der Datenbank lesen
        SqlDB dataReader = new SqlDB("select orgID, topOrgID, displayName, displayNameShort from structure where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            orgDisplayName = dataReader.getString(2);
            orgDisplayNameShort = dataReader.getString(3);
        ...
        dataReader.close();
        // OrgManager
        dataReader = new SqlDB("select number, numberTotal, numberSum, numberSumInput, status, backComparison, locked from orgmanager_structure where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberEmployees = dataReader.getInt32(0);
            numberEmployeesTotal = dataReader.getInt32(1);
        ...
        dataReader.close();
        values.Clear();
        dataReader = new SqlDB("SELECT fieldID, value FROM orgmanager_structurefieldsvalues WHERE orgID='" + OrgID + "'", projectID);
        while (dataReader.read())
        {
            TKeyValue tempKeyValue = new TKeyValue();
            tempKeyValue.key = dataReader.getString(0);
        ...
        // Anzahl eingetragen Mitarbeiter der Einheit ermitteln
        dataReader = new SqlDB("SELECT COUNT(employeeID) FROM orgmanager_employee WHERE orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberEmployeesInput = dataReader.getInt32(0);
        }
        dataReader.close();
        // FollowUp
        dataReader = new SqlDB("select workshop, mail, livepdf, maxpdfLevel, communicationfinished, communicationfinsheddate, participants from followup_structure where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            workshop = dataReader.getBool(0);
            mailAdress = dataReader.getString(1);
        ...
        // Download
        // Response
        dataReader = new SqlDB("select number from response_structure where orgID='" + OrgID + "'", projectID);
        if (dataReader.read())
        {
            numberIst = dataReader.getInt32(0);
        }
        ...
        if (expandedNav)
        {
            foreach (TAdminStructure tempStructure in childs)
            {
    ...
    public void getChilds(string aProjectID)
    {
        awayChilds();
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure where topOrgID='" + this.OrgID + "' ORDER BY displayName", projectID);
        while (dataReader.read())
        {
            TAdminStructure newChild = new TAdminStructure(dataReader.getInt32(0), this, this.level + 1, aProjectID);
    ...
    /// <summary>
    /// Ermittlung der OrgID aller untergeordneter Organisationseinheiten über alle Level; die Start-Organisationseinheit wird nicht eingeschlossen
    /// </summary>
    ...
    public static void getOrgIDList(int aOrgID, ref ArrayList aOrgIDList, string aProjectID)
    {
        // schreibt die OrgIDs aller Untereinheten von aOrgID in aOrgIDList
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure where topOrgID='" + aOrgID + "'", aProjectID);
        while (dataReader.read())
        {
            int childOrgID = dataReader.getInt32(0);
            aOrgIDList.Add(childOrgID);
            // der Aufruf wird rekursiv ausgeführt
            getOrgIDList(childOrgID, ref aOrgIDList, aProjectID);
        }
        dataReader.close();
    }
    /// <summary>
    ...
    {
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure where topOrgID='" + aOrgID.ToString() + "'", aProjectID);
        while (dataReader.read())
        {
            int singleSollOrg = 0;
            int singleSollRes = 0;
        ...
        dataReader = new SqlDB("select number, numberSumInput from orgmanager_structure where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
            aSollOrg = aSollOrg + dataReader.getInt32(0);
        }
        dataReader.close();
        dataReader = new SqlDB("select count(employeeID) from orgmanager_employee where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
            aIstOrgInputEmployee = aIstOrgInputEmployee + dataReader.getInt32(0);
        }
        dataReader.close();
        dataReader = new SqlDB("select number, target, numberPaper from response_structure where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
        ...
        dataReader.execSQLwithParameter("UPDATE orgmanager_structure SET numbersum=@numbersum, numbersumInput=@numbersumInput WHERE orgID=@orgID", parameterList);
        ...
        parameterList.addParameter("orgID", "int", aOrgID.ToString());
        dataReader = new SqlDB(aProjectID);
        dataReader.execSQLwithParameter("UPDATE response_structure SET numberSum=@numberSum, targetSum=@targetSum WHERE orgID=@orgID", parameterList);
    }
    /// <summary>
    ...
    public static void recalc(string aProjectID)
    {
        ...
        // Root-Object finden
        SqlDB dataReader;
        dataReader = new SqlDB("select orgID from structure where topOrgID='0'", aProjectID);
        if (dataReader.read())
        {
    ...
    protected static void getCounterBounces(int aOrgID, ref int aIsBounce, ref int aIsBounceSum, string aProjectID)
    {
        int singleBounces = 0;
        ...
        while (dataReader.read())
        {
            int singleBouncesSum = 0;
            // rekursiver Aufruf
            getCounterBounces(dataReader.getInt32(0), ref singleBounces, ref singleBouncesSum, aProjectID);
            aIsBounceSum = aIsBounceSum + singleBouncesSum;
        }
        dataReader.close();
        // Werte der Einheit ermitteln und hinzuaddieren
        dataReader = new SqlDB("select count(employeeID) from orgmanager_employee where orgID='" + aOrgID.ToString() + "' AND isBounce='1'", aProjectID);
        if (dataReader.read())
        {
            aIsBounce = dataReader.getInt32(0);
            aIsBounceSum = aIsBounceSum + aIsBounce;
        }
        dataReader.close();

        // Summen in DB zurückschreiben
        TParameterList parameterList = new TParameterList();
        parameterList.addParameter("bounces", "int", aIsBounce.ToString());
        parameterList.addParameter("bouncesSum", "int", aIsBounceSum.ToString());
        parameterList.addParameter("orgID", "int", aOrgID.ToString());
        dataReader = new SqlDB(aProjectID);
        dataReader.execSQLwithParameter("UPDATE orgmanager_structure SET bounces=@bounces, bouncesSum=@bouncesSum WHERE orgID=@orgID", parameterList);
    }

    public static void recalcBounces(string aProjectID)
    {
        // Initialisierung der Summen
        int isBounes = 0;
        int isBouncesSum = 0;
        ...
        {
            int rootID = dataReader.getInt32(0);
            getCounterBounces(rootID, ref isBounes, ref isBouncesSum, aProjectID);
        }
        dataReader.close();
    }
    ...
    public static bool getTopOrgIDList(int aOrgID, ref ArrayList aOrgIDList, string aProjectID)
    {
        ...
        while (aOrgID != 0)
        {
            dataReader = new SqlDB("select topOrgID from structure where orgID='" + aOrgID + "'", aProjectID);
            if (dataReader.read())
            {
    ...
    public static void getOrgName(int aOrgID, ref string displayName, ref string displayNameShort, string aProjectID)
    {
        ...
        displayNameShort = "";
        SqlDB dataReader;
        dataReader = new SqlDB("select displayname, displaynameshort from structure where orgID='" + aOrgID.ToString() + "'", aProjectID);
        if (dataReader.read())
        {
